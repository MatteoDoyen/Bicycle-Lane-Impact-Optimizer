...
#include "../header/djikstra_cuda.h"
#include <stdbool.h>
#include <stdlib.h>


void allocate_graph_on_gpu(vertex_t **graph_gpu, const vertex_t **graph_cpu , uint32_t num_vertices){
    ...
    for (uint32_t i = 0; i < num_vertices; i++) {
        cudaMalloc(&(graph_gpu[i]->in_edges), DEFAULT_NEIGHBOURS * sizeof(edge_t *));
        cudaMemcpy(graph_gpu[i]->in_edges, graph_cpu[i]->in_edges, DEFAULT_NEIGHBOURS * sizeof(edge_t *), cudaMemcpyHostToDevice);

        cudaMalloc(&(graph_gpu[i]->out_edges), DEFAULT_NEIGHBOURS * sizeof(edge_t *));
        cudaMemcpy(graph_gpu[i]->out_edges, graph_cpu[i]->out_edges, DEFAULT_NEIGHBOURS * sizeof(edge_t *), cudaMemcpyHostToDevice);
    }
}

void allocate_edge_on_gpu(){
...
void free_path_on_gpu(path_t *d_path)
{
    ...
    cudaFree(d_path->djikstra_sp);
    cudaFree(d_path->visibilite);
}

void allocate_path_on_gpu(path_t *d_path, const path_t *h_path)
{
    ...
    cudaMemcpy(&(d_path->nb_djikstra_sp), &(h_path->nb_djikstra_sp), sizeof(uint32_t), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(&(d_path->djikstra_dist), &(h_path->djikstra_dist), sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(&(d_path->cps_djikstra_dist), &(h_path->cps_djikstra_dist), sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(&(d_path->cps_djikstra_danger), &(h_path->cps_djikstra_danger), sizeof(double), cudaMemcpyHostToDevice);

    // Allocate memory for arrays on GPU
    CUDA_SAFE_CALL(cudaMalloc((void **)&(d_path->chemin), h_path->nb_chemin * sizeof(uint32_t)));
    CUDA_SAFE_CALL(cudaMalloc((void **)&(d_path->djikstra_sp), h_path->nb_djikstra_sp * sizeof(uint32_t)));
    CUDA_SAFE_CALL(cudaMalloc((void **)&(d_path->visibilite), h_path->nb_visibilite * sizeof(uint32_t)));
}

double djikstra_forward_cuda(vertex_t **graph, int nb_vertices, double **dist_array_ref, int *parents, path_t *path)
{
    ...
    CUDA_SAFE_CALL(cudaMalloc((void**)&gpu_nb_vertices, (sizeof(int))));
    CUDA_SAFE_CALL(cudaMemcpy(gpu_nb_vertices, &nb_vertices, sizeof(int), cudaMemcpyHostToDevice));
    
    allocate_path_on_gpu(&gpu_path, path);
    allocate_graph_on_gpu(gpu_graph,graph,nb_vertices);
    CUDA_SAFE_CALL(cudaMalloc((void **)&gpu_dist_array, dist_array_size));
    CUDA_SAFE_CALL(cudaMalloc((void **)&gpu_marked_vertex, marked_vertex_size));
    CUDA_SAFE_CALL(cudaMalloc((void **)&gpu_parents, parents_size));
    CUDA_SAFE_CALL(cudaMalloc((void **)&gpu_path, parents_size));
    ...
    CUDA_SAFE_CALL(cudaMemcpy(gpu_dist_array, dist_array, dist_array_size, cudaMemcpyHostToDevice));
    CUDA_SAFE_CALL(cudaMemcpy(gpu_marked_vertex, marked_vertex, marked_vertex_size, cudaMemcpyHostToDevice));
    CUDA_SAFE_CALL(cudaMemcpy(gpu_parents, parents, parents_size, cudaMemcpyHostToDevice));

    CUDA_SAFE_CALL(cudaEventRecord(exec_start));
    for (int i = 0; i < nb_vertices; i++)
    {
        min_distance <<<gridMin, blockMin>>>(gpu_dist_array, gpu_marked_vertex, gpu_closest_vertex, nb_vertices);                            // find min
    ...
    CUDA_SAFE_CALL(cudaFree(gpu_dist_array));
    CUDA_SAFE_CALL(cudaFree(gpu_marked_vertex));
    CUDA_SAFE_CALL(cudaFree(gpu_parents));
...
__global__ void cudaRelax(vertex_t **graph, double *dist_array, int *parents, bool *marked_vertex, int *global_min_index, path_t *path)
{
    ...
    edge_t *edge = get_edge_if_exist(graph, source, next);
    double current_cost = cost_function(&path->profil, &edge->dist, &edge->danger);

    if (edge != NULL &&
        vertex_is_in_visibilite(path, next) &&
        !marked_vertex[next] &&
