...
bool edge_is_in_visibilite(trace_t *traces, edge_t *edge)
{
    ...
    for (unsigned int i = 0; i < traces->nb_visibilite; i++)
    {
        ...
        // then returns true
        if (traces->visibilite[i] == edge->succ->id)
        {
            succ_in=true;
        }
        else if (traces->visibilite[i] == edge->pred->id)
        {
            pred_in=true;
        }
        if(pred_in && succ_in){
...
bool troncon_is_in_trace(trace_t *traces, edge_t *edge)
{
    ...
    {
        if (traces->chemin[i] == edge->succ->id || traces->chemin[i] == edge->pred->id)
        {
            // printf("chem %d succ %d pred%d\n",traces->chemin[i],edge->succ->id,edge->pred->id);
            // printf("origin %d dest %d id %d\n",traces->origin,traces->destination,edge->id);
            return true;
        }
    }
    for (unsigned int i = 0; i < traces->nb_visibilite; i++)
    {
        if (traces->visibilite[i] == edge->succ->id || traces->visibilite[i] == edge->pred->id)
        {
            // printf("chem %d succ %d pred %d\n",traces->visibilite[i],edge->succ->id,edge->pred->id);
            return true;
        }
    }
    return false;
...
void get_traces(char *csv_path, char *csv_delimiter,trace_t ** traces_ref, int *nb_traces)
{
    ...
    (*traces_ref) = calloc(1,sizeof(trace_t) * nb_row);
    trace_t *traces = (*traces_ref);
