...
double djikstra_forward(struct vertex_t **graph, int nb_vertices, double **dist_array_ref, int *parents, path_t *path)
{
    ...
    while (verticesToVisit != NULL)
    {
        ...
        for (unsigned int edge_i = 0; edge_i < graph[min_dist_index]->nb_edges_out; edge_i++)
        {
            dest_vertex_id = graph[min_dist_index]->out_edges[edge_i]->succ->id;
            if (!vertex_is_in_visibilite(path, dest_vertex_id))
            {
                continue;
            }
            if (!markedVertex[dest_vertex_id] && graph[min_dist_index]->out_edges[edge_i] != NULL && (dist_array[min_dist_index] + cost_function(&path->profil, &graph[min_dist_index]->out_edges[edge_i]->dist, &graph[min_dist_index]->out_edges[edge_i]->danger) < dist_array[dest_vertex_id]))
            {
                dist_array[dest_vertex_id] = dist_array[min_dist_index] + cost_function(&path->profil, &graph[min_dist_index]->out_edges[edge_i]->dist, &graph[min_dist_index]->out_edges[edge_i]->danger);
                if(parents!=NULL){
...
double djikstra_backward(struct vertex_t **graph, int nb_vertices, double **dist_array_ref, int *parents, path_t *path)
{
    ...
    while (verticesToVisit != NULL)
    {
        ...
        // Update dist value of the adjacent vertices
        for (unsigned int edge_i = 0; edge_i < graph[min_dist_index]->nb_edges_in; edge_i++)
        {
            dest_vertex_id = graph[min_dist_index]->in_edges[edge_i]->pred->id;
            if (!vertex_is_in_visibilite(path, dest_vertex_id))
            {
                continue;
            }
            if (!markedVertex[dest_vertex_id] && graph[min_dist_index]->in_edges[edge_i] != NULL && (dist_array[min_dist_index] + cost_function(&path->profil, &graph[min_dist_index]->in_edges[edge_i]->dist, &graph[min_dist_index]->in_edges[edge_i]->danger) < dist_array[dest_vertex_id]))
            {
                dist_array[dest_vertex_id] = dist_array[min_dist_index] + cost_function(&path->profil, &graph[min_dist_index]->in_edges[edge_i]->dist, &graph[min_dist_index]->in_edges[edge_i]->danger);
                if(parents!=NULL){
                    parents[dest_vertex_id] = min_dist_index ;
                }
