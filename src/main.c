...
int main()
{
   ...
   double elapsed_time_thread;
    ...
    for (int i = 100; i <= 400; i+=100)
    {
        ...
        get_edges_to_optimize_for_budget(200,GRAPH_FILE_NAME,PATHS_FILE_NAME,&selected_edges);
        clock_gettime(CLOCK_MONOTONIC, &end);
        elapsed_time_mono = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
        free_select_edges(selected_edges);

        printf("lest go thread\n");
        // clock_gettime(CLOCK_MONOTONIC, &start);
        // get_edges_to_optimize_for_budget_threaded(200,GRAPH_FILE_NAME,PATHS_FILE_NAME,12,&selected_edges);
