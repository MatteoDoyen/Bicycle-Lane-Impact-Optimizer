...
double dijkstra_forward_small_graph_omp(struct vertex_t **graph, int nb_vertices, double **dist_array_ref, int *parents, path_t *path)
{
    bool markedVertex[nb_vertices];
    ...
    while (verticesToVisit != NULL)
    {
        ...
        #pragma omp parallel for private(current_edge, dest_vertex_id, current_cost)
